"""
Модели пользователей и ролей системы СпортРядом.

Что здесь:
- User: базовая модель на UUID (AbstractBaseUser). Четыре роли:
  client / owner / manager / administrator.
- Client: расширение для клиентов — паспортные данные (series, number,
  issue_date, department_code), required при регистрации (152-ФЗ / NDA).
- Owner: расширение для владельцев инвентаря — паспорт + банковские реквизиты.
- OwnerAgreement: согласие владельца с пользовательским соглашением платформы.
- BankAccount: банковский счёт владельца для выплат.
- Manager: менеджер с привязкой к городу/региону.
- Administrator: полный доступ к административной панели.
- PassportNDA: запись о принятии NDA на обработку паспортных данных
  (152-ФЗ). Версия документа + дата принятия.

Связано с:
- users/views.py: регистрация с обязательным NDA-чекбоксом
- custom_admin/views.py: owner_passport_registry — реестр паспортов (152-ФЗ)
- rentals/models.py: Rental.client / Rental.owner — FK на профили ролей
- inventory/models.py: Inventory.owner — FK на Owner

Ключевые слова: пользователь, роли, клиент, владелец, менеджер, паспорт, 152-ФЗ, NDA
"""
...
class Manager(models.Model):
    ...
    is_super_manager = models.BooleanField(default=False, verbose_name='Супер-менеджер')
