...
def generate_rental_contract(
    *,
    ...
    _sub(doc, '3.4',
         'Оплата производится через электронную систему платформы «СпортРядом».')
    ...
    # Ст. 622 ГК РФ: арендная плата за время просрочки возврата
    _sub(doc, '4.2',
         'В соответствии со статьёй 622 Гражданского кодекса Российской Федерации '
         'при несвоевременном возврате Инвентаря Арендатор обязан вносить арендную '
         'плату за всё время просрочки. Размер платы за каждый день просрочки '
         'возврата Инвентаря равен суточной арендной плате, установленной '
         'п. 3.1 настоящего договора. Если указанная плата не покрывает '
         'причинённых Арендодателю убытков, он вправе потребовать их возмещения.')
    _sub(doc, '4.3',
...
def generate_owner_contract(
    *,
    ...
    _para(doc,
          ...
          f'и {manager_full_name}, действующий от имени платформы «СпортРядом» (далее — Платформа), '
    ...
    _sub(doc, '1.1',
         ...
         f'{inventory_name} (далее — Инвентарь) на платформе «СпортРядом» третьим лицам.')
    _sub(doc, '1.2',
