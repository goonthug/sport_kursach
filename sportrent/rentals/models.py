"""
Модели системы аренды спортивного инвентаря.

Что здесь:
- Rental: главная модель аренды. Поля просрочки: overdue_fee_paid_at,
  overdue_fee_snapshot (фиксируем сумму на момент выявления — ст. 622 ГК РФ).
  additional_payment_paid — доплата за продление по инициативе менеджера.
- PaymentHistory: аудит-лог всех платёжных событий по аренде
- Reservation: предварительное бронирование до подтверждения менеджером
- Payment: устаревший мок-платёж (заменён PaymentIntent), оставлен для истории
- Contract: договор аренды — привязан к Rental и Owner
- DamageReport: акт повреждения инвентаря после возврата

Связано с:
- payments/models.py: PaymentIntent → Rental (FK) — реальные платежи ЮКассы
- rentals/views.py: управление статус-машиной аренды
- contracts/generator.py: генерация .docx договора для Contract
- inventory/models.py: Inventory → Rental (FK) — какой инвентарь арендуется

Ключевые слова: аренда, просрочка, штраф, доплата, продление, статья 622 ГК РФ
"""
